...
class SavingItemsBloc extends Bloc<SavingItemsEvent, SavingItemsState> {
  ...
  SavingItemsBloc({required SavingItemRepository savingItemRepository})
    ...
    on<GetSavingItemsOverviewEvent>((event, emit) async {
      try {
        final savingItemsGroupedByDate =
            await _savingItemRepository.queryAllSavingItemsGroupedByDate();
        emit(SavingItemsState(
            SavingItemStateStatus.success, savingItemsGroupedByDate));
      } catch (err) {
    ...
    on<AddSavingItemEvent>((event, emit) async {
      try {
        final savingItems =
            await _savingItemRepository.addSavingItem(event.savingItem);
